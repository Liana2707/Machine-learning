...
import optuna
from local_objectives import *
from local_callbacks import XGBTimer, lgb_timer
...
def start_optimization(lib, n_trials, param_info=None, param_info_fork=None, cat_features=None, **objective_kwargs):
    ...
    if lib in ('lgb', 'xgb', 'cb', 'cb_gpu'):
        ...
        sampler = optuna.samplers.TPESampler(n_startup_trials=int(0.9 * n_trials), n_ei_candidates=33, multivariate=True)
        study = optuna.create_study(directions=['maximize', 'minimize'], sampler=sampler) # score, time
    elif lib in ('lgb_msf'):
        study = optuna.create_study(directions=['maximize', 'maximize']) # score, score
    elif lib in ('lgb_mtf'):
        study = optuna.create_study(directions=['minimize', 'minimize']) # time, time
        
    if lib == 'lgb':
        func = partial(lgb_objective, param_info=param_info, **objective_kwargs)
    elif lib == 'lgb_rs':
        func = partial(lgb_rs_objective, param_info=param_info, **objective_kwargs)
    elif lib == 'xgb':
        ...
        func = partial(lgb_multitime_fork_objective, param_info=param_info, param_info_fork=param_info_fork,
                       **objective_kwargs)
    elif lib in ('cb', 'cb_gpu'):
        func = partial(сb_objective, param_info=param_info, cat_features=cat_features, **objective_kwargs)
    elif lib == 'cb_rs':
        func = partial(сb_rs_objective, param_info=param_info, cat_features=cat_features, **objective_kwargs)
